...
model_name = 'single-net-{}-bone-{}-bn-{}-epoch-{}-batch-{}-lr-{}-dim-{}-train-{}-rot-{}-set-{}-ext-{}-loss-{}-up-{}-filters-{}-red_factor-{}-pyr_agg-{}-bk-{}-fl_weight-{}-fv-{}'.format(args.net_type,\
		 	args.backbone, args.bn, args.epoch, args.batch_size, args.lr, args.dim,\
...
model = unet1(input_size = (None,None,3), nb_classes = n_classes, base_filters = 16, bn = args.bn, act_name = 'softmax')
...
# 		vols = vols[:,:,:,1] if vols.shape[-1] >= 2 else vols[:,:,:,0]
		...
		for si in range(vols.shape[0]):
				slice = vols[si,:,:,:]
				rows.append(slice)
				if si%6 == 5 and not si == vols.shape[0]-1:
		...
		def on_epoch_end(self, epoch, logs=None):
				...
				if epoch%5 == 0:
						...
						for i in range(0, len(valid_dataset),int(len(valid_dataset)/36)):
								...
								img_vols.append(np.uint8(255*(img-img.min())/(img.max()-img.min())))
						...
						save_images(model_folder+'/epoch-{}-gt.png'.format(epoch), gt_map)
						save_images(model_folder+'/epoch-{}-pr.png'.format(epoch), pr_map)
...
# model = unet1(nb_classes = n_classes, base_filters = 16, bn = args.bn, act_name = 'softmax')
# model.compile(optimizer=optim, loss=total_loss, metrics = metrics)
