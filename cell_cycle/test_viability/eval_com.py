...
parser.add_argument("--model_file", type=str, default = 'com_models.txt')
...
model_root_folder = '/data/viability_c_models/'
nb_train_test = 200
...
# 	subset = subsets[0]
	...
	## reshape
	offset = 16
	pr_masks = pr_masks[:,offset:-offset,offset:-offset,:]
	gt_masks = gt_masks[:,offset:-offset,offset:-offset,:]
...
# 	save = True
	...
	with open(result_dir+'/{}_summary_{}.txt'.format(subset, epoch), 'w+') as f:
		# save iou and dice
		f.write('iou_classes: {:.4f},{:.4f},{:.4f},{:.4f}; mIoU: {:.4f}\n'.format(iou_classes[0],iou_classes[1],iou_classes[2], iou_classes[3], mIoU))
		f.write('dice_classes:{:.4f},{:.4f},{:.4f},{:.4f}; mDice: {:.4f}\n'.format(dice_classes[0],dice_classes[1],dice_classes[2], dice_classes[3], mDice))
